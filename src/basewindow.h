...
// https://msdn.microsoft.com/en-us/library/windows/desktop/ff381400.aspx
...
public:
  static LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam,
    ...
    if (uMsg == WM_NCCREATE) {
      ...
      p = reinterpret_cast<DERIVED_TYPE *>(
  ...
  bool Create(LPCWSTR caption, DWORD style, DWORD style_ex = 0,
              int x = CW_USEDEFAULT, int y = CW_USEDEFAULT,
              int width = CW_USEDEFAULT, int height = CW_USEDEFAULT,
              HWND parent = nullptr, LPCWSTR menu = nullptr) {
    if (!windowclass) {
      WNDCLASSEXW wcex = {};
    ...
    if (!hwnd_) {
      hwnd_ = ::CreateWindowExW(
          style_ex, ClassName(), caption, style, x, y, width, height, parent,
          /*hMenu*/ nullptr, GetModuleHandle(nullptr), this);
    }
    return !!hwnd_;
  }
...
template <class DERIVED_TYPE> ATOM BaseWindow<DERIVED_TYPE>::windowclass = 0;
